(function() {
  ...
  var animationsMixin = {
    ...
    methods: {
      ...
      // certification items
      animCertificationItems() {
        const certificationItems = gsap.utils.toArray(".certification-section .certification-items li");

        if (!certificationItems.length) {
          return;
        }

        const certificationItemsTL = gsap.timeline({
          scrollTrigger: {
            trigger: ".certification-items",
            start: "top 82%",
            end: "top 50%",
            scrub: 0.3
          }
        });
        certificationItems.forEach((el, i) => {
          const pos = i === 0 ? "" : "< +=0.2";
          certificationItemsTL.from(el, {
            autoAlpha: 0
          }, pos).from(el, {
            y: 50
          }, "<");
  ...
  const app = Vue.createApp({
    ...
    data() {
      return {
        ...
        portfolioItems: [], 
        // current page of certification items
        certificationItemsPage: 1,
        // certification items per page
        certificationItemsPerPage: 6,
        // certification items filter by type
        certificationFilters: ['All', '.Net', 'React'],
        certificationCurrentFilter: 'All',
        // certification archive name
        certificationArchiveName: '',
        ...
        allCertificationItems:[{
          ...
          imgUrl:"assets/images/icons/certifications/Udemy-Logo.png",
          issueDate:"June 2021",
          credId:"UC-673d8eca-137d-42e8-bf3f-e9624a4c7e80",
          credURL:"https://udemy-certificate.s3.amazonaws.com/pdf/UC-673d8eca-137d-42e8-bf3f-e9624a4c7e80.pdf",
          ...
          skills: ['ASP.NET Core','Entity Framework (EF) Core','HTML','Design Patterns','C#','JSON','REST APIs','Git Hub' ,' MVC','TypeScript','.NET Core','SQL Database'],
          category: '.Net'
        },{
          id:3,
          title:{
          ...
          credURL:"https://udemy-certificate.s3.amazonaws.com/pdf/UC-871ff761-0ff4-4b8e-a3ca-e1241793f7b5.pdf",
          skills: ['React','PostgreSQL', 'Express.js' , 'Node.js', 'Git Hub', 'HTML', 'CSS', 'JavaScript', 'render'],
          category: 'React'
        },{
          id:4,
          title:{
            en:"Build an app with ASPNET Core and Angular from scratch",
            ar:""
          },
          IssueOrg:{
            en:"Udemy",
          ...
          issueDate:"Ongoing",
          credId:"-",
          credURL:"https://www.udemy.com/course/build-an-app-with-aspnet-core-and-angular-from-scratch/",
          skills: ['Angular','.Net Core','Git Hub' , 'HTML' , 'CSS', 'JavaScript', "C#",'Entity Framework (EF) Core','SQLite' ],
          category: '.Net'
         },]//.reverse()
         , 
        certificationItems:[],
    ...
    methods: {
      ...
      initializeTilt() {
       ...
       /*  const certificationItems = this.$refs.certificationItems; */
       ...
       /*  VanillaTilt.init(certificationItems.querySelectorAll('.certification-item'), {
          max: 8,
          speed: 400,
          glare: true,
          'max-glare': 0.3
      ...
      getPortfolioItems() {
        ...
        getCertificationItems() {
          console.log("GET CERTIFICATION ITEMS METHOD")
          const itemsArr = this.allCertificationItems
           .filter(item => {
           /* const urlParams = new URLSearchParams(window.location.search);
            const tax = urlParams.get('tax');  
            ...
            if (tax) {
              if (tax === 'cat') {
                const cat = urlParams.get('cat');
                this.certificationArchiveName = cat;
                return item.category === cat;
              } else if (tax === 'tools') {
                const tool = urlParams.get('tools');
                this.certificationArchiveName = tool;
                return item.tools.includes(tool);
              }
            } else {
              
            }*/return this.certificationCurrentFilter === 'All' || item.category === this.certificationCurrentFilter;
          })  
          .slice(this.filteredCertificationItems.length, this.certificationItemsPage * this.certificationItemsPerPage);
          console.log(itemsArr)

           // check if have works or not
        if (itemsArr.length) {
          this.certificationItems.push(...itemsArr);
          this.$nextTick(() => {
            // reinitialize VanillaTilt for new items
            this.certificationItemsPage > 1 ;
            
            // && this.initializeTilt(); Forces the ScrollTrigger instance to re-calculate its start and end values
            setTimeout(() => ScrollTrigger.refresh(), 500);
          });
          this.certificationItemsPage++;
        } else {
          // show message "No works" to the user
          this.setNotify({
            className: 'danger',
            msg: this.$refs.certificationItems.getAttribute('data-no-works-msg'),
            time: 3000
          });
        }
        },
      ...
      filterCertificationItems(filter) {
        this.certificationCurrentFilter = filter;
        this.certificationItemsPage = 1;

        if (this.filteredCertificationItems.length) {
          this.$nextTick(() => {
            // reinitialize VanillaTilt for new items
            this.certificationItemsPage > 1 && this.initializeTilt(); // Forces the ScrollTrigger instance to re-calculate its start and end values
    ...
    computed: {
      ...
      // filtered certification items
      filteredCertificationItems() {
        const urlParams = new URLSearchParams(window.location.search);
        const tax = urlParams.get('tax');
        ...
        if (tax) {
          ...
          return this.certificationItems.filter(item => this.certificationCurrentFilter === 'All' || item.category === this.certificationCurrentFilter);
        }
      },
      ...
      getSinglePortfolioItem() {
       ...
       // get single certification item
       getSingleCertificationItem(id) {
        const urlParams = new URLSearchParams(window.location.search);
        /* const id = urlParams.get('id'); */
        return this.allCertificationItems.find(item => item.id == id);
      },
