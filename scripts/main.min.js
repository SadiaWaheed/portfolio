    ...
    const app = Vue.createApp({
      ...
      methods: {
        ...
        initializeTilt() {
          ...
          const certificationItems = this.$rerfs.certificationItems;
        ...
        getPortfolioItems() {
          const itemsArr = this.allPortfolioItems.filter(item => {
            ...
            if (tax) {
              ...
              } else if(tax === 'projectLinks'){
                const link = urlParams.get('projectLinks');
                this.portfolioArchiveName = link;
                return item.projectLinks.includes(link);
              }
            } else {
        ...
        getCertificationItems() {
          const itemsArr = this.allCertificationItems.filter(item => {
            const urlParams = new URLSearchParams(window.location.search);
            const tax = urlParams.get('tax');
            ...
            if (tax) {
              if (tax === 'cat') {
                ...
                const link = urlParams.get('projectLinks');
                this.certificationArchiveName = link;
                return item.projectLinks.includes(link);
              }
            } else {
              return this.currentFilter === 'All' || item.category === this.currentFilter;
            }
          }).slice(this.filteredCertificationItems.length, this.certificationItemsPage * this.itemsPerPage); // check if have works or not
